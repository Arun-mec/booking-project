...
const Card = ({ image, content }) => {
  return (
    <div className="card flex flex-col items-center justify-start w-[18vw] p-2">
      <img src={image} alt="No image" className="h-44 w-full object-cover border-white border-[1px]" />
      <span className="text-white text-md font-normal font-sans italic pt-2">
        {content}
      </span>
    </div>
  );
...
const Bannercard = ({bannerData}) => {
  return (
    <div className="carousel-card flex flex-col items-start my-8 w-full">
      <div className="title">
        <h6 className="font-bold text-7xl text-white text-opacity-80">
          {bannerData.title}
        </h6>
      </div>
      <div className="cards flex flex-row py-3">
        ...
        <Card image={bannerData.img3} content={bannerData.body3} />
      </div>
      <div
